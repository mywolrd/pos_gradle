...
public class JdbcItemDao implements ItemDao {
    ...
    public _Item uniqueByNameAndType(String name, String type) {
        ...
        try {
            return this.namedParameterJdbcTemplate.queryForObject(uniqueByNameAndType, parameter, new BeanPropertyRowMapper<>(_Item.class));
    ...
    public List<_Item> listItems() {
        try {
            return this.namedParameterJdbcTemplate.query(listAll, new BeanPropertyRowMapper<>(_Item.class));
        } catch (DataAccessException e) {
    ...
    public List<_Item> listItemsByType(String type) {
        ...
        try {
            return this.namedParameterJdbcTemplate.query(listByType, parameter, new BeanPropertyRowMapper<>(_Item.class));
        } catch (DataAccessException e) {
            return new ArrayList<>();
        }
    }
    ...
    public void save(_Item item) {
        SqlParameterSource parameter = new MapSqlParameterSource().addValue(DBNames.TYPE, item.getType()).addValue(DBNames.NAME, item.getName())
                .addValue(DBNames.DOLLAR, item.getDollar()).addValue(DBNames.CENT, item.getCent()).addValue(DBNames.ACTIVE, Boolean.TRUE);
        try {
    ...
    public void update(_Item item) {
        SqlParameterSource parameter = new MapSqlParameterSource().addValue(DBNames.TYPE, item.getType()).addValue(DBNames.NAME, item.getName())
                .addValue(DBNames.DOLLAR, item.getDollar()).addValue(DBNames.CENT, item.getCent()).addValue(DBNames.ID, item.getId());
        try {
